...
class ConstructorResolver {
	...
	/**
	 * 根据mbd的是否允许访问非公共构造函数和方法标记
	 * 【{@link RootBeanDefinition#isNonPublicAccessAllowed}】来获取factoryClass的所有候选方法
	 *
	...
	private Method[] getCandidateMethods(Class<?> factoryClass, RootBeanDefinition mbd) {
		// 如果有系统安全管理器
		if (System.getSecurityManager() != null) {
			// 使用特权方式执行:如果mbd允许访问非公共构造函数和方法
			// ，就返回factoryClass子类和其父类的所有声明方法，首先包括子类方法；
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		Class<?> factoryClass;
		// 定义一个表示是静态工厂方法的标记: 标记是实例工厂还是静态工厂
		boolean isStatic;
		...
		if (factoryBeanName != null) {
			...
			if (mbd.isSingleton() && this.beanFactory.containsSingleton(beanName)) {
				// 这个时候意味着Bean工厂中已经有beanName的Bean对象
				// ，而这个还要生成多一个Bean名为BeanName的Bean对象，导致了冲突
		...
		// 如果为空的话，就要去找出factoryClass的以及父类的所有的方法
		// ，进一步找出方法修饰符一致且名字跟工厂方法名字相同的且是bean注解的方法，并放入列表里。
			...
			if (factoryMethodToUse == null || argsToUse == null) {
				...
				// 如果resolvedValues不为null
					...
					for (ValueHolder value : valueHolders) {
						// 如果value的参数类型不为null，就获取该参数类型的简单类名；
						// 否则(如果value的参数值不为null，即获取该参数值的简单类名;否则为"null")
	...
	private int resolveConstructorArguments(String beanName, RootBeanDefinition mbd, BeanWrapper bw,
		...
		for (ConstructorArgumentValues.ValueHolder valueHolder : cargs.getGenericArgumentValues()) {
			...
			if (valueHolder.isConverted()) {
				...
				Object resolvedValue =
						valueResolver.resolveValueIfNecessary("constructor argument", valueHolder.getValue());
				// 使用valueHolder所封装的type,name属性以及解析出来的resovledValue
				// 构造出一个ConstructorArgumentValues.ValueHolder对象
	...
	private ArgumentsHolder createArgumentArray(
		...
		// 定义一个用于存储构造函数参数值Holder，以查找下一个任意泛型参数值时
		// ，忽略该集合的元素的HashSet,初始化长度为paramTypes的数组长度
	...
	private Object[] resolvePreparedArguments(String beanName, RootBeanDefinition mbd, BeanWrapper bw,
		...
		for (int argIndex = 0; argIndex < argsToResolve.length; argIndex++) {
			...
			// 如果argValue是String对象
				...
				// 评估beanDefinition中包含的argValue,如果argValue是可解析表达式
				// ，会对其进行解析，否则得到的还是argValue
	...
	private static class ArgumentsHolder {
		...
		public int getAssignabilityWeight(Class<?>[] paramTypes) {
			...
			for (int i = 0; i < paramTypes.length; i++) {
				...
				if (!ClassUtils.isAssignableValue(paramTypes[i], this.arguments[i])) {
					// 返回Integer最大值，意味着既然连最终的转换后参数值都不能匹配
					// ，这个情况下的paramTypes所对应的工厂方法是不可以接受的
			...
			// 在完全匹配的情况下，返回Integer最大值-1024；意味着因为最终的转换后参数值和原始参数值都匹配
			// ，这种情况下paramTypes所对应的工厂方法非常可以接收
		...
		public void storeCache(RootBeanDefinition mbd, Executable constructorOrFactoryMethod) {
			...
			synchronized (mbd.constructorArgumentLock) {
				// 让mbd的已解析的构造函数或工厂方法【{@link RootBeanDefinition#resolvedConstructorOrFactoryMethod}】
				// 引用constructorOrFactoryMethod
