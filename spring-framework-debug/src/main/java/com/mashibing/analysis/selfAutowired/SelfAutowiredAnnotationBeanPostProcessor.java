...
/**
 * description  SelfAutowiredAnnotationBeanPostProcessor <BR>
 * <p>
 * author: zhao.song
 ...
 * version 1.0
 */
public class SelfAutowiredAnnotationBeanPostProcessor extends InstantiationAwareBeanPostProcessorAdapter
	...
	public SelfAutowiredAnnotationBeanPostProcessor() {
		this.autowiredAnnotationTypes.add(SelfAutowired.class);
...
//		}
		...
		do {
			...
			ReflectionUtils.doWithLocalFields(targetClass, field -> {
				...
				if (ann != null) {
					...
					//查看是否是required的
					boolean required = determineRequiredStatus(ann);
					currElements.add(new SelfAutowiredAnnotationBeanPostProcessor.AutowiredFieldElement(field, required));
				}
			});
			...
			ReflectionUtils.doWithLocalMethods(targetClass, method -> {
				...
				if (ann != null && method.equals(ClassUtils.getMostSpecificMethod(method, clazz))) {
					...
					currElements.add(new SelfAutowiredAnnotationBeanPostProcessor.AutowiredMethodElement(method, required, pd));
				}
			});
	...
	private class AutowiredFieldElement extends InjectionMetadata.InjectedElement {
		...
		protected void inject(Object bean, @Nullable String beanName, @Nullable PropertyValues pvs) throws Throwable {
			...
			// 如果缓存，从缓存中获取
			if (this.cached) {
				// 如果  cachedFieldValue instanceof DependencyDescriptor
				// 。则调用 resolveDependency 方法重新加载。
						...
						if (value != null || this.required) {
							...
							if (autowiredBeanNames.size() == 1) {
								...
								if (beanFactory.containsBean(autowiredBeanName) &&
									...
									this.cachedFieldValue = new SelfAutowiredAnnotationBeanPostProcessor.ShortcutDependencyDescriptor(
							...
							if (autowiredBeans.size() == argumentCount) {
								...
								for (int i = 0; i < paramTypes.length; i++) {
									...
									if (beanFactory.containsBean(autowiredBeanName) &&
										...
										cachedMethodArguments[i] = new SelfAutowiredAnnotationBeanPostProcessor.ShortcutDependencyDescriptor(
