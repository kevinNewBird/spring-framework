...
public abstract class AbstractMessageConverterMethodProcessor extends AbstractMessageConverterMethodArgumentResolver
	...
	protected <T> void writeWithMessageConverters(@Nullable T value, MethodParameter returnType,
		...
		if (value instanceof CharSequence) {
			...
			body = value;
			// 获取返回结果的类型（返回值 body 不为空则直接获取其类型
			// ，否则从返回结果类型 returnType 获取其返回值类型)
		...
		// <3.1.1> 如果存在 ContentType 的值，并且不包含通配符
		// ，则使用它作为 selectedMediaType
		...
		if (isContentTypePreset) {
			...
			// 如果 body 非空，并且无可产生的 MediaType 数组
			// ，则抛出 HttpMediaTypeNotAcceptableException 异常
			if (body != null && producibleTypes.isEmpty()) {
				throw new HttpMessageNotWritableException(
						"No converter found for return value of type: " + valueType);
			}
			// 通过 acceptableTypes 来比对，将符合的 producibleType
			// 添加到 mediaTypesToUse 结果数组中
		...
		// 如果到达此处，并且 body 非空，说明没有匹配的 HttpMessageConverter 转换器
		// ，则抛出 HttpMediaTypeNotAcceptableException 异常
		if (body != null) {
