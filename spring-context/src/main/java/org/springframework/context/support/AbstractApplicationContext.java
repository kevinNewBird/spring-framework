...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void setParent(@Nullable ApplicationContext parent) {
		// tip: 当时spring mvc或者其他整合了spring的框架，这里将会发生整合merge
		// 也就是父子容器
		this.parent = parent;
		if (parent != null) {
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 子类覆盖方法做额外的处理,此处我们自己一般不做任何的扩展工作,但是可以查看web中的代码,是由具体实现的(系统环境变量注册到容器中)
				...
				// 国际化处理,在springmvc的时候通过国际化的代码重点讲
				initMessageSource();
	...
	protected void prepareRefresh() {
		...
		// earlyApplicationListeners: 运行之前的监听器
		// applicationListeners: 运行过程中的监听器
		if (this.earlyApplicationListeners == null) {
			//TIP: applicationListeners在某些情况下不为空!比如在spring-boot脚手架中
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		...
		// TIP: 在我们的容器启动之前,有可能已经启动了另一个bean工厂了,所以叫刷新
		refreshBeanFactory();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 设置beanFactory的表达式语言处理器(配置以及解析器)
		...
		// 为什么要忽略?
		// ignoreDependencyType 和  ignoreDependencyInterface 的区别?
		...
		// 设置几个自动装配的特殊规则,当在进行ioc初始化的如果有几个实现,那么就使用指定的对象进行注入
		// 类似@Primary注解, 优先注入的设置(当调用时如果没有指定,默认使用该对象实例)
		...
		// 编译器织入是指在java编译器，采用特殊的编译器，将切面织入到java类中，而类加载期织入则指通过特殊的类加载器，
		// 在类字节码加载到JVM时，织入切面，运行期织入则是采用cglib和jdk进行切面的织入
	...
	protected void initMessageSource() {
		...
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			...
			// MessageSource此接口提供了处理消息的策略，包含了信息的国际化和包含参数的信息的替换
	...
	protected void initApplicationEventMulticaster() {
		...
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		// 判断容器中是否存在bdName为applicationEventMulticaster的bd，也就是说自定义的事件监听多路广播器
		// ，必须实现ApplicationEventMulticaster接口.默认不包含applicationEventMulticaster
...
//			getApplicationEventMulticaster().addApplicationListener(this.getBean(listenerBeanName,ApplicationListener.class));
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果beanFactory之前没有注册嵌入值解析器，则注册默认的嵌入值解析器，主要用于注解属性值的解析[invokeBeanFactoryPostProcessors]
		if (!beanFactory.hasEmbeddedValueResolver()) {
			beanFactory.addEmbeddedValueResolver(strVal -> getEnvironment().resolvePlaceholders(strVal));
		}

		// Initialize LoadTimeWeaverAware beans early to allow for registering their transformers early.
		// 尽早初始化loadTimeWeaverAware bean,以便尽早注册它们的转换器[AOP]
