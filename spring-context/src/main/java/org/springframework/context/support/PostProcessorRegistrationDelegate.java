...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		// 判断beanFactory是否是BeanDefinitionRegsitry类型,此处是DefaultListableBeanFactory
		// ,实现了实现了BeanDefinitionRegistry接口,所以为true
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			// 最后，调用入参beanFactoryPostProcessors中的
			// 普通BeanFactoryPostProcessor的postProcessBeanFactory方法
		...
		// 遍历实现了PriorityOrdered接口的BeanFactoryPostProcessor
	...
	private static void invokeBeanDefinitionRegistryPostProcessors(
		...
		for (BeanDefinitionRegistryPostProcessor postProcessor : postProcessors) {
			//调用 postProcessor 的 postProcessBeanDefinitionRegistry以使得postProcess
			// 往registry注册BeanDefinition对象
	...
	private static void invokeBeanFactoryPostProcessors(
		...
		for (BeanFactoryPostProcessor postProcessor : postProcessors) {
			...
			// ，使得每个postProcessor对象都可以对beanFactory进行调整
	...
	private static final class BeanPostProcessorChecker implements BeanPostProcessor {
		...
		/**
		 * description   后置处理器的before方法，什么都不做，直接返回对象  <BR>
		 *
		...
		/**
		 * description   后置处理器的after方法，用来判断哪些是不需要检测的bean  <BR>
		 *
		  * @param bean:the new bean instance
		 * @param beanName: the name of the bean
		 * @return {@link Object}
		...
		/**
		 * description   判断 beanName 是否是 完全内部使用  <BR>
		 *
		  * @param beanName:
		 * @return {@link boolean}
		 */
